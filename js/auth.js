...
} catch(e) { console.error(‘showApp display error:’, e); return; }

try {
const isA = CU.role === ‘admin’;
document.getElementById(‘upill’).innerHTML =
`<div class="uav" style="background:${isA?'rgba(0,212,255,.15)':'rgba(0,255,136,.1)'};color:${isA?'var(--cy)':'var(--gn)'}">${(CU.fullname||'?')[0]}</div> <span style="font-size:11px">${CU.fullname}</span>`;
document.getElementById(‘ni-users’).style.display = isA ? ‘flex’ : ‘none’;
const sbam = document.getElementById(‘sbam’);
if (sbam) sbam.style.display = can(‘add’) ? ‘block’ : ‘none’;
} catch(e) { console.error(‘showApp header error:’, e); }

try { checkMob(); } catch(e) {}

// render after DOM settles
requestAnimationFrame(() => {
try {
tab = ‘devices’; devFlt = ‘all’; repFlt = ‘all’; SQ = ‘’;
rSidebar();
rPage();
rHeader();
} catch(e) {
console.error(‘showApp render error:’, e);
// Show error in page
const pg = document.getElementById(‘PC’);
if (pg) pg.innerHTML = ’<div style="padding:20px;color:#ff4466">خطأ في التحميل: ’ + e.message + ‘</div>’;
}
});
}
...
document.getElementById(‘hstats’).innerHTML = ` <div class="hst"><div class="v" style="color:var(--cy)">${devs.length}</div><div class="l">أجهزة</div></div> <div class="hst"><div class="v" style="color:var(--yw)">${repAct}</div><div class="l">صيانة</div></div> <div class="hst"><div class="v" style="color:var(--gn);font-size:10px">${IQD(todaySales)}</div><div class="l">اليوم</div></div>`;
...
`<div class="mi ${amod==='all'?'active':''}" onclick="setAM('all')"> <div class="mi-i"><div class="mi-n">🗂 كل المخزن</div></div> <span class="mi-c">${devs.length}</span> </div>` +
...
return `<div class="mi ${amod===m.id?'active':''}" onclick="setAM('${m.id}')"> <div class="mi-i"> <div class="mi-n">${m.brand} ${m.name}</div> <div class="mi-s">${m.size?m.size+'"':''} ${m.ver||''}</div> <div class="mbar"><div class="mbar-f" style="width:${pct}%;background:${col}"></div></div> </div> <span class="mi-c">${cnt}</span> </div>`;
}).join(’’);
}
