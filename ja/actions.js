...
var opts = CP.map(function(p){return ‘<option value="'+p+'">’;}).join(’’);
var rmCode = “this.closest(’.prow’).remove()”;
div.innerHTML = ‘<input type="text" value="'+esc(name)+'" placeholder="القطعة" list="cp_'+id+'" style="flex:1;min-width:0">’
+ ‘<datalist id="cp_'+id+'">’+opts+’</datalist>’
+ ‘<select>’
...
+ ‘</select>’
+ ‘<button class="rmb" onclick="'+rmCode+'">×</button>’;
pe.appendChild(div);
}
...
`<div class="ti" onclick="this.querySelector('input').click()"> <input type="checkbox" id="tp_${i}" data-part="${esc(p.name)}"> <label for="tp_${i}">🟢 ${p.name}</label> </div>`).join(’’);
...
var reps = Object.values(DB.repairs||{}), sales = Object.values(DB.sales||{});
var devRows = Object.values(DB.devices||{}).map(function(d){
var m=(DB.models||{})[d.modelId]||{brand:’?’,name:’?’};
var g=(d.parts||[]).filter(function(p){return p.status===‘good’;}).map(function(p){return p.name;}).join(’، ‘);
var b=(d.parts||[]).filter(function(p){return p.status===‘bad’;}).map(function(p){return p.name;}).join(’، ‘);
return ‘<tr><td>’+m.brand+’ ‘+m.name+’</td><td>’+(d.name||’’)+’</td><td>’+(d.done?‘✅’:‘⏳’)+’</td><td style="color:green">’+g+’</td><td style="color:red">’+b+’</td></tr>’;
}).join(’’);
var stMap={new:‘جديدة’,inprog:‘نشطة’,done:‘مكتملة’,cancelled:‘ملغاة’,waiting:‘انتظار’};
var repRows = reps.map(function(r){
return ‘<tr><td>#’+r.id.slice(-5)+’</td><td>’+r.customerName+’</td><td>’+(r.deviceBrand||’’)+’ ‘+(r.deviceModel||’’)+’</td><td>’+(stMap[r.status]||’’)+’</td><td>’+(r.agreedPrice?IQD(r.agreedPrice):’’)+’</td></tr>’;
}).join(’’);
var total = IQD(sales.reduce(function(a,s){return a+s.price;},0));
var html = ‘<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>ALMALIK تقرير</title>’
+ ‘<style>body{font-family:Arial;padding:20px;font-size:12px}h1,h2{color:#1d4ed8;margin-bottom:8px}’
+ ‘table{width:100%;border-collapse:collapse;margin-bottom:16px;font-size:11px}’
+ ‘th,td{border:1px solid #e5e7eb;padding:5px 8px;text-align:right}th{background:#eff6ff;font-weight:700}’
+ ‘@media print{button{display:none}}</style></head><body>’
+ ‘<h1>📋 ALMALIK — تقرير شامل</h1>’
+ ‘<p>’+new Date().toLocaleDateString(‘ar-SA’)+’ | أجهزة: ‘+Object.values(DB.devices||{}).length+’ | صيانات: ‘+reps.length+’ | مبيعات: ‘+total+’</p>’
+ ‘<h2>الأجهزة</h2><table><tr><th>الموديل</th><th>الاسم</th><th>الحالة</th><th>سليمة</th><th>تالفة</th></tr>’+devRows+’</table>’
+ ‘<h2>صيانات الزبائن</h2><table><tr><th>#</th><th>الزبون</th><th>الجهاز</th><th>الحالة</th><th>السعر</th></tr>’+repRows+’</table>’
+ ‘<h2>إجمالي: ‘+total+’</h2>’
+ ‘<button onclick="window.print()" style="background:#1d4ed8;color:#fff;border:none;padding:9px 18px;border-radius:6px;cursor:pointer">🖨️ طباعة</button>’
+ ‘</body></html>’;
